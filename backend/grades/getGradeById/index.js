// backend/grades/getGradeById/index.js
...
const baseHandler = async (event) => {
  try {
    ...
    const res = await docClient.send(new GetCommand({ TableName: TABLE.GRADES, Key: { id } }))
    if (!res.Item) return error('Không tìm thấy điểm', 404)
